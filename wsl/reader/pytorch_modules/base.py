...
# from wsl.common.torch_utils import load_ort_optimized_hf_model
...
class WSLReaderBase(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(transformer_model, PosixPath):
        ...
        if isinstance(transformer_model, str):
            ...
            if "wsl-reader" in config.model_type:
        ...
        self.wsl_reader_model = transformer_model

        self.wsl_reader_model_config = self.wsl_reader_model.config
    ...
    def forward(self, **kwargs) -> Dict[str, Any]:
        return self.wsl_reader_model(**kwargs)
    ...
    def tokenizer(self) -> tr.PreTrainedTokenizer:
        ...
        self._tokenizer = tr.AutoTokenizer.from_pretrained(
            self.wsl_reader_model.config.name_or_path
            if self.wsl_reader_model.config.name_or_path
            else self.wsl_reader_model.config.transformer_model
        )
        return self._tokenizer
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if transformer_model.__class__.__name__ not in WSL_READER_CLASS_MAP:
            raise ValueError(f"Model type {type(transformer_model)} not recognized.")

        reader_class = WSL_READER_CLASS_MAP[transformer_model.__class__.__name__]
    ...
    def save_pretrained(
        ...
        **kwargs,
        ...
        # save the model
        self.wsl_reader_model.config.register_for_auto_class()
        self.wsl_reader_model.register_for_auto_class()
        self.wsl_reader_model.save_pretrained(
