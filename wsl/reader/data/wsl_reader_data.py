...
from wsl.reader.data.wsl_reader_data_utils import (
    add_noise_to_value,
    batchify,
    chunks,
    flatten,
...
def preprocess_sample(
    wsl_sample: WSLReaderSample,
    tokenizer,
    lowercase_policy: float,
    add_topic: bool = False,
) -> None:
    if len(wsl_sample.tokens) == 0:
        return

    if lowercase_policy > 0:
        lc_tokens = np.random.uniform(0, 1, len(wsl_sample.tokens)) < lowercase_policy
        wsl_sample.tokens = [
            t.lower() if lc else t for t, lc in zip(wsl_sample.tokens, lc_tokens)
        ]
    ...
    offsets_mapping = [
        [
            (
                ss + wsl_sample.token2char_start[str(i)],
                se + wsl_sample.token2char_start[str(i)],
            )
            for ss, se in tokenization_out.offset_mapping[i]
        ]
        for i in range(len(wsl_sample.tokens))
    ]
    ...
    if add_topic:
        topic_tokens = tokenizer(
            wsl_sample.doc_topic, add_special_tokens=False
        ).input_ids
        topic_offset = len(topic_tokens)
        wsl_sample.topic_tokens = topic_offset
    ...
    for tok, cs in wsl_sample._d["token2char_start"].items():
        char2token_start[str(cs)] = tok

    char2token_end = dict()
    for tok, ce in wsl_sample._d["token2char_end"].items():
        char2token_end[str(ce)] = tok
    token2word_start = {
        str(i): int(char2token_start[str(s)])
        for i, (s, _) in enumerate(offsets_mapping, start=topic_offset)
        if str(s) in char2token_start
    }
    token2word_end = {
        str(i): int(char2token_end[str(e)])
        for i, (_, e) in enumerate(offsets_mapping, start=topic_offset)
        if str(e) in char2token_end
    }
    wsl_sample._d.update(
        dict(
    ...
    if "window_labels" in wsl_sample._d:
        wsl_sample.window_labels = [
            (s, e, l.replace("_", " ")) for s, e, l in wsl_sample.window_labels
        ]
...
class WSLDataset(IterableDataset):
    def __init__(
        self,
        ...
        samples: Optional[Iterator[WSLReaderSample]] = None,
        ...
        **kwargs,
        ...
        self.samples: Optional[List[WSLReaderSample]] = None
    ...
    def dataset_iterator_func(self):
        skipped_instances = 0
        data_samples = (
            load_wsl_reader_samples(self.dataset_path)
    ...
    def preshuffle_elements(self, dataset_elements: List):
        ...
        dataset_elements = sorted(
            dataset_elements, key=lambda x: len(x["predictable_candidates"])
        )

        if self.for_inference:
            return dataset_elements

        ds = list(chunks(dataset_elements, 128))
    ...
    def convert_to_char_annotations(
        ...
        for (
            ...
            span_start = sample.token2char_start[str(span_start)]
            span_end = sample.token2char_end[str(span_end)]
            titles = [title for title, _ in candidates_probs]
            # remove duplicates preserving order
            titles = list(dict.fromkeys(titles))
            char_probs_annotations[(span_start, span_end)] = titles
